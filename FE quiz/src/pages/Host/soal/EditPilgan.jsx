...
const EditQuestionPage = () => {
    ...
    const handleSubmit = async () => {
        ...
        try {
            const response = await fetch(`${api}/updateDataQuiz/${quizId}/${questionId}`, {
                method: 'PUT',
                body: formData,
            });
            ...
            <div className='col-md-10 '>
                ...
                <Box sx={{ p: 5, backgroundColor: 'none' }}>
                    ...
                    <Box sx={{ mt: 5, display: 'flex', justifyContent: 'center' }}>
                        ...
                        >
                            <img
                                src={previewImage || (questionImage ? `http://localhost:4001/image/${questionImage}` : defaultImage)}
                                alt="Insert Media"
                                width="200"
                                height="200"
                        ...
                        {['primary', 'warning', 'success', 'error'].map((color, index) => (
                            <Grid item xs={12} md={6} key={index}>
                                ...
                                >
                                    ...
                                    >
                                      <img
                                            src={newOptionImages[index] || (originalOptionImages[index] ? `http://localhost:4001/image/${originalOptionImages[index]}` : defaultImage)}
                                            alt={`Option ${index + 1}`}
                                            width="70"
                                            height="70"
